defmodule Argos.CLI do
  ...
  defp exec_command(_opts, args) do
    ...
    if command do
      result = Argos.exec_command(command, remaining_args)
  ...
  defp sudo_command(_opts, args) do
    ...
    if command do
      result = Argos.exec_sudo(command)
  ...
  defp parallel_command(opts, args) do
    ...
    if length(tasks_from_opts) > 0 do
      ...
      result = Argos.run_parallel(tasks_from_opts, max_concurrency: concurrency)
  ...
  defp kill_command(_opts, args) do
    ...
    if process_name do
      result = Argos.kill_process(process_name)
  ...
  defp ps_command() do
    ...
    result = Argos.exec_command("ps", ["aux"])
  ...
  defp log_command(opts, args) do
    ...
    Argos.log(level, message, metadata)
    System.halt(0)
  end
